			...
			<section class='row'>
				...
				<div class='col-md-10 col-sm-10'>
						...
						elseif(isset($_GET['administratives'])){
								...
								if(isset($_POST['add_session_btn'])){
									...
									if(empty($add_session)){
										...
										$query = " SELECT * FROM `sessions` WHERE `sessions` = '".mysqli_real_escape_string($connection, $add_session)."' ";
										$run_query = mysqli_query($connection, $query);
										if(mysqli_num_rows($run_query) > 0){
											...
											$query = " INSERT INTO `sessions`(`sessions`, `date_added`) VALUES('".mysqli_real_escape_string($connection, $add_session)."', '$date') ";
											$run_query = mysqli_query($connection, $query);
											if($run_query == true){
					...
					elseif(isset($_GET['students_reg_no'])){
						$select = "Select a class";  // Default value
						if (isset($_POST['reg_no_class'])) {
								$select = $_POST['reg_no_class'];  // Set to the selected class if the form was submitted
						}
											...
											if($run_query == true){
												if(mysqli_num_rows($run_query) > 0){
													...
													while($result = mysqli_fetch_assoc($run_query)){
														...
														$full_name = ucwords("{$student_firstname} {$student_lastname}");
					...
					elseif(isset($_GET['manage_subjects'])){
						$select = "Select a subject";  // or you could use a default subject
											...
											<div class='col-md-6'>
												<div class='input-group'>
													...
													<select class='form-control' name='specify_senior_subjects'>
														...
														<option selected >{$select}</option>";
															$senior_subject_array = array("Application Development", "Human Computer", "Database Management System", "Functional English", "Networking", "Object Oriented Programming", "Programming");
							...
							<div class='panel panel-primary'>
								<div class='panel-body'>
									<div class='col-md-2 thumbnail text-center'>
										<img src='img/ll.jpg' class='img-responsive img-rounded' />
									</div>
