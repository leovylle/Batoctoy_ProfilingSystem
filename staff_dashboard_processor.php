...
<?php
	...
	if(isset($_POST['student_reg_btn'])){
		...
		$gen_password = "bt_dbms";
					...
					$image_location = "image/none.jpg"; // Default value
...
if (isset($name) && !empty($name)) {
    if (move_uploaded_file($tmp_name, $location.$name)) {
        $image_location = $location.$name;
						...
						$reg_number = $class_code .$school_name .$increase_student;
						...
						if(mysqli_num_rows($run_query) == 1){
							...
							$query = "INSERT INTO `students`(
								`gender`, `firstname`, `lastname`, `dob`, `mob`, `yob`, `contact_phone`, `address`, `state`, `nationality`, `class`, `reg_number`, `gen_password`, `passport`, `date_of_reg`
						) VALUES (
								'$reg_student_gender', '$reg_student_firstname', '$reg_student_lastname', '$reg_student_dob', '$reg_student_mob', '$reg_student_yob', '$reg_student_phone', '$reg_student_address', '$reg_student_state', '$reg_student_nationality', '$reg_student_tc', '$reg_number', '$gen_password', '$image_location', '$date'
						)";						
